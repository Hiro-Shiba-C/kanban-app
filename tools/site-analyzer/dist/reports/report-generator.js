...
class ReportGenerator {
     ...
     * コンソール形式のレポートを生成
     */
    async generateConsoleReport(result) {
        console.log('\n' + chalk_1.default.bold.blue('📊 サイト構造解析レポート'));
     ...
     * 依存関係統計を出力
     */
    printDependencyStatistics(result) {
        console.log(chalk_1.default.bold('\n🔗 依存関係統計'));
     ...
     * コンポーネント統計を出力
     */
    printComponentStatistics(result) {
        console.log(chalk_1.default.bold('\n⚛️  React コンポーネント統計'));
    ...
    printIssues(result) {
        ...
        console.log(chalk_1.default.bold('\n⚠️  検出された問題'));
        const errors = issues.filter(i => i.severity === analysis_types_1.IssueSeverity.ERROR);
        const warnings = issues.filter(i => i.severity === analysis_types_1.IssueSeverity.WARNING);
        const infos = issues.filter(i => i.severity === analysis_types_1.IssueSeverity.INFO);
        if (errors.length > 0) {
        ...
        if (infos.length > 0) {
            ...
            infos.slice(0, 5).forEach(issue => {
                console.log(chalk_1.default.blue(`    • ${issue.message}`));
            });
        }
    }
    /**
    ...
    printRecommendations(result) {
        ...
        if (recommendations.length === 0) {
            console.log(chalk_1.default.green('  現在のところ、特別な推奨事項はありません。'));
            return;
        }
        recommendations.forEach((rec, index) => {
            console.log(chalk_1.default.blue(`  ${index + 1}. ${rec}`));
        });
    }
    /**
     * 推奨事項を生成
    ...
    generateSummary(result) {
        return {
            ...
            issueCount: {
                errors: result.issues.filter(i => i.severity === analysis_types_1.IssueSeverity.ERROR).length,
                warnings: result.issues.filter(i => i.severity === analysis_types_1.IssueSeverity.WARNING).length,
                infos: result.issues.filter(i => i.severity === analysis_types_1.IssueSeverity.INFO).length
            },
    ...
    calculateHealthScore(result) {
        let score = 100;
        const errors = result.issues.filter(i => i.severity === analysis_types_1.IssueSeverity.ERROR).length;
        const warnings = result.issues.filter(i => i.severity === analysis_types_1.IssueSeverity.WARNING).length;
        score -= errors * 10;
        score -= warnings * 3;
